...
public class ClusterGen { 
	...
	public int m_noisecounter; //number of segment points that are classified as noise (ie not part of a dense component. Does not include any components later excluded for failing the number of trajectories or cluster count tests
	
	public ArrayList<LineSegmentId> m_idArray = new ArrayList<ClusterGen.LineSegmentId>();  //list of segment IDs
	public ArrayList<CMDPoint> m_lineSegmentPointArray = new ArrayList<CMDPoint>(); //list of segment points, 2n-d objects with start/end point
	
	// used for performing the DBSCAN algorithm
	public static final int UNCLASSIFIED = -2;
	public static final int NOISE = -1;

	public static final int MIN_LINESEGMENT_LENGTH = 175; //originally 50
	public static final int m_minTrajectories = 5; //the minimum number of different trajectories a cluster must be made of 
	...
	public boolean performDBSCAN(double eps, int minLns)  {
		...
		// Print component allocation array
		...
		try {
			fos = new FileOutputStream(m_document.inputFile + "_RegionAllocation_" + eps + "_" + minLns  + "len_" + MIN_LINESEGMENT_LENGTH + ".txt");
			osw = new OutputStreamWriter(fos);
			bw = new BufferedWriter(osw);
			
			
			
			bw.write(m_document.inputFile + "\n" + "epsParam:"+ eps +" minLnsParam:"+minLns+" minlength: " + MIN_LINESEGMENT_LENGTH + " mintrajs: " + m_minTrajectories + "\n");
			int trajid = 0;
			for (int i = 0; i < m_componentIdArray.size(); i++) {
				
				if (m_idArray.get(i).trajectoryId != trajid) {
					trajid++;
					bw.write("\n");
								
				}
				
				bw.write(m_componentIdArray.get(i) + " ");
	...
	private boolean storeClusterComponentIntoIndex() {
		...
		int too_short_segments = 0;
		
		m_nTotalLineSegments = 0;
		ArrayList<Double> distances = new ArrayList<Double>(); 
		//System.out.println("trajlist dims: " + m_document.m_trajectoryList.get(0).getM_nDimensions());
		int nTotalPartitionPoints = 0;
		int nTotalPoints = 0;
		for (int i = 0; i < m_document.m_trajectoryList.size(); i++) {
			Trajectory pTrajectory = m_document.m_trajectoryList.get(i);
			nTotalPartitionPoints = nTotalPartitionPoints + pTrajectory.getM_nPartitionPoints();
			nTotalPoints = nTotalPoints + pTrajectory.getM_nPoints();
			...
			for (int j =0; j<pTrajectory.getM_nPartitionPoints()-1; j++) {
				...
				endPoint = pTrajectory.getM_partitionPointArray().get(j + 1);
				 ...
				 distances.add(measureDistanceFromPointToPoint(startPoint, endPoint));  // Make array of distances to check min_linesegment_length paramaterisation
				 
				
				
				//System.out.println("trajectory " + i + ", point " + j + ", length " + measureDistanceFromPointToPoint(startPoint, endPoint));
		 ...
		 //Writes distances array out 
		/*
		try
		{
		    String filename= "seg_lengths_2D.txt";
		    FileWriter fw = new FileWriter(filename,true); //the true will append the new data
		    
		    for (int i = 0; i < distances.size(); i++ ) {
		    	  fw.write(distances.get(i) + "\n"); 
		...
		*/
        ...
        System.out.print("Average point distance is: " + average + "\n");
       
        
        
        Collections.sort(distances);
        double median = distances.size()/2;
       ...
       System.out.print("Median point distance is: " + median + "\n");
        */
		
		System.out.println("total points: " + nTotalPoints + ", total partition points: " + nTotalPartitionPoints + ", accepted segments: " + m_lineSegmentPointArray.size() + ", too short segments: " + too_short_segments);
		//System.out.println("segpt dims: " + m_lineSegmentPointArray.get(0).getM_nDimensions());
		return true;
	}
	...
	boolean  constructLineSegmentCluster() {
		...
		for (int i = 0; i < m_currComponentId; i++) {	
			...
			m_lineSegmentClusters[i].nLineSegments = 0; //num line segments in this cluster I think
			m_lineSegmentClusters[i].nClusterPoints = 0;
			m_lineSegmentClusters[i].nTrajectories = 0;
			m_lineSegmentClusters[i].enabled = false;
			m_lineSegmentClusters[i].failreason = "0";
		}
		//  ... END

		//  accumulate the direction vector of a line segment, i.e. for each component get the accumulated direction vector
		for (int i = 0; i < m_nTotalLineSegments; i++) {
		...
		//  compute the average direction vector for each line segment cluster
		//  START ...
		...
		for (int i = 0; i < m_currComponentId; i++) {
			...
			//  a line segment cluster must have trajectories more than the minimum threshold, the code default for this was minlns parameter
			if (clusterEntry.nTrajectories >= m_minTrajectories) {
	...
	boolean constructLineSegmentListOnly() {
		// designed for use with the 3D data, we don't want to make the representative trajectories, we just want a small amount of info about the regions
		...
		for (int i = 0; i < m_nTotalLineSegments; i++) {
			int componentId = m_componentIdArray.get(i);
			int trajid = m_idArray.get(i).trajectoryId; 
			if (componentId >= 0) {
				for (int j = 0; j < nDimensions; j++) {
					double difference = m_lineSegmentPointArray.get(i).getM_coordinate(nDimensions + j)
							- m_lineSegmentPointArray.get(i).getM_coordinate(j);
				...
				if (!m_lineSegmentClusters[componentId].trajectoryIdList.contains(trajid)) {m_lineSegmentClusters[componentId].trajectoryIdList.add(trajid); 
				m_lineSegmentClusters[componentId].nTrajectories++;
				}
			} else {m_noisecounter++;
			}
	...
	private void computeRepresentativeLines(LineSegmentCluster clusterEntry) {
		...
		while (iter != (clusterEntry.candidatePointList.size()-1) && clusterEntry.candidatePointList.size() > 0)	{
			...
			if ((int)(lineSegments.size()) >= m_minTrajectories ) // originally m_minLnsParam   
			{
		...
		if (nClusterPoints >= 2) {
			...
			if (clusterEntry.failreason == "0")  { 
				clusterEntry.failreason = "insufficient points";
			} 
			//if already failed due to trajectories keep that info over the alternate 
			
			clusterEntry.enabled = false;
			
			/* I want to keep the info here, but can still flag it as a concern */
			/*clusterEntry.candidatePointList.clear();
			clusterEntry.clusterPointArray.clear();
			clusterEntry.trajectoryIdList.clear(); */
			}
	...
	private void computeAndRegisterClusterPoint(
		...
		//clusterPoint.setM_coordinate(2, 5.0); 
		//NJA need to update this part with the value for the new  coordinate in 3D
	...
	private void getSweepPointOfLineSegment(LineSegmentCluster clusterEntry,
		...
		//  NOTE: this program code works only up to the 3-dimensional data, and doesn't fully work for 3D NJA
	...
	private void RegisterAndUpdateLineSegmentCluster(int componentId, int lineSegmentId) {
		...
		//  NOTE: this program code works only up to 3-dimensional data and does not consider the z dimension NJA
	...
	private boolean storeLineSegmentCluster() {
		...
		for (int i = 0; i < m_currComponentId; i++) {
			...
			/* removed requirement for clusters to be "enabled" to get the outputs, because I want to know their details anyway */
			...
			pClusterItem.setM_nLineSegments(m_lineSegmentClusters[i].nLineSegments);	
			pClusterItem.set_enabledstatus(m_lineSegmentClusters[i].failreason);
